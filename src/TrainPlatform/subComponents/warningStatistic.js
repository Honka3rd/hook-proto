...
class WarningStatistic extends React.Component {
	warningStatisticId = 'warning_statistic';
	warningStatisticEcharts = null;
	constructor(props){
		super(props);
		this.state = {
	...
	componentDidMount(){
		const warningStatisticComponent = document.getElementById(this.warningStatisticId);
		this.warningStatisticEcharts = Echart.init(warningStatisticComponent);
		this.warningStatisticEcharts.setOption(this.warningStatisticOption);
	...
	componentDidUpdate(preprops,prestates){
		if(prestates.mainHight!==this.state.mainHight || prestates.mainWidth!==this.state.mainWidth ){
			this.warningStatisticEcharts.resize();
		}
	}
		...
		series: [
			{
				...
				data: [
					{value: 100, name: '紧急100'},
					{value: 300, name: '一般300'},
				]
			}
		]
	}
